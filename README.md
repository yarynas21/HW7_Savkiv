...
This task implements a two-container system that performs logging and basic alerting based on incoming user messages.

## 🧩 Architecture

The system consists of two microservices:
- **api_service**: Accepts incoming POST requests and logs all inputs.
- **alert_service**: Analyzes incoming messages, detects sensitive patterns, and generates alert reports if needed.

Services communicate via HTTP using Docker Compose.

```
User → api_service → alert_service
                      ↓
           logs/app.log + error_reports/*.txt
```

### 2. Logging
We initially implemented logging using the built-in Python logging system (Approach 1), which stored logs in `.txt` files. Later, we upgraded to **Approach 2** by integrating **InfluxDB**, a time-series database.

Each log is sent to:
- `InfluxDB` at `localhost:8086`
- Still mirrored to local files:
  - `api_service/logs/app.log`
  - `alert_service/logs/app.log`

### 3. Alerts
- `alert_service` detects and logs the following:
  - Keywords: `"password"`, `"secret"`, `"credentials"`
  - Fraud attempts: `"fraud"`, `"scam"`, `"fake"`
  - Email addresses (regex)
  - Card-like numbers (10+ digits)

- For each alert, a `.txt` report is generated inside:
  ```
...
- Alert reports will appear in `alert_service/error_reports/`
- Logs will be saved in both:
  - `logs/app.log` (local)
  - InfluxDB (`log_event` measurement)

### 🔎 Verify InfluxDB logs
...
**Generated alert reports folder:**  
![](reports.png)
# HW7  
## Task 1: Logging and Alerts
